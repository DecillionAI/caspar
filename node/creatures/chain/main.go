...
func process(input string) string {
	...
	hostReq("microPutJson", map[string]any{"key": "Json::CreatureNamespace::chain", "path": "lastInput", "data": p.Payload, "merge": true})
	if p.Path != "" {
		hostReq("microPutLink", map[string]any{"key": "creatureNamespace::chain::lastPath", "value": p.Path})
	}
	out, _ := json.Marshal(map[string]any{"ok": true, "namespace": "chain"})
