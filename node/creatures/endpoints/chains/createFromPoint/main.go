...
func process(input string) string {
	...
	if "createFromPoint" == "create" || "createFromPoint" == "createFromPoint" || "createFromPoint" == "createShard" {
		hostReq("microGenId", map[string]any{"source": "chains.createFromPoint"})
	}
	hostReq("microPutJson", map[string]any{
		"key":   "Json::CreatureEndpoint::chains::createFromPoint",
		"path":  "lastInput",
		"data":  p.Payload,
		"merge": true,
	})
	if p.UserID != "" {
		hostReq("microPutLink", map[string]any{"key": "creatureEndpoint::chains::createFromPoint::lastUser", "value": p.UserID})
	}
	if p.PointID != "" {
	...
	out, _ := json.Marshal(map[string]any{"ok": true, "endpoint": "/chains/createFromPoint"})
