...
func process(input string) string {
	...
	if "createFromStore" == "create" || "createFromStore" == "createFromStore" || "createFromStore" == "createShard" {
		hostReq("microGenId", map[string]any{"source": "chains.createFromStore"})
	}
	...
	if p.UserID != "" {
		hostReq("microPutLink", map[string]any{"key": "creatureEndpoint::chains::createFromStore::lastUser", "value": p.UserID})
	}
	if p.StoreID != "" {
	...
	out, _ := json.Marshal(map[string]any{"ok": true, "endpoint": "/chains/createFromStore"})
