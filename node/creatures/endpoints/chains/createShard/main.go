...
func process(input string) string {
	...
	if "createShard" == "create" || "createShard" == "createFromStore" || "createShard" == "createShard" {
		hostReq("genId", map[string]any{"source": "chains.createShard"})
	}
	...
	if p.UserID != "" {
		hostReq("dbOp", map[string]any{"op": "put", "key": "creatureEndpoint::chains::createShard::lastUser", "val": p.UserID})
	}
	if p.StoreID != "" {
