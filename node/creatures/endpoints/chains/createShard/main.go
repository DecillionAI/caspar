...
func process(input string) string {
	...
	if "createShard" == "create" || "createShard" == "createFromPoint" || "createShard" == "createShard" {
		hostReq("microGenId", map[string]any{"source": "chains.createShard"})
	}
	...
	if p.UserID != "" {
		hostReq("microPutLink", map[string]any{"key": "creatureEndpoint::chains::createShard::lastUser", "value": p.UserID})
	}
	if p.PointID != "" {
	...
	out, _ := json.Marshal(map[string]any{"ok": true, "endpoint": "/chains/createShard"})
