...
func process(input string) string {
	...
	if "registerNode" == "create" || "registerNode" == "createFromStore" || "registerNode" == "createShard" {
		hostReq("microGenId", map[string]any{"source": "chains.registerNode"})
	}
	...
	if p.UserID != "" {
		hostReq("microPutLink", map[string]any{"key": "creatureEndpoint::chains::registerNode::lastUser", "value": p.UserID})
	}
	if p.StoreID != "" {
	...
	out, _ := json.Marshal(map[string]any{"ok": true, "endpoint": "/chains/registerNode"})
