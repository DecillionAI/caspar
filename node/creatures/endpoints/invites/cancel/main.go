...
func process(input string) string {
	...
	if "cancel" == "create" || "cancel" == "createFromPoint" || "cancel" == "createShard" {
		hostReq("microGenId", map[string]any{"source": "invites.cancel"})
	}
	hostReq("microPutJson", map[string]any{
		"key":   "Json::CreatureEndpoint::invites::cancel",
		"path":  "lastInput",
		"data":  p.Payload,
		"merge": true,
	})
	if p.UserID != "" {
		hostReq("microPutLink", map[string]any{"key": "creatureEndpoint::invites::cancel::lastUser", "value": p.UserID})
	}
	if p.PointID != "" {
	...
	out, _ := json.Marshal(map[string]any{"ok": true, "endpoint": "/invites/cancel"})
