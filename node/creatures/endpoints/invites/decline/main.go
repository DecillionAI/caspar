...
func process(input string) string {
	...
	if "decline" == "create" || "decline" == "createFromStore" || "decline" == "createShard" {
		hostReq("microGenId", map[string]any{"source": "invites.decline"})
	}
	hostReq("microPutJson", map[string]any{
		"key":   "Json::CreatureEndpoint::invites::decline",
		"path":  "lastInput",
		"data":  p.Payload,
		"merge": true,
	})
	if p.UserID != "" {
		hostReq("microPutLink", map[string]any{"key": "creatureEndpoint::invites::decline::lastUser", "value": p.UserID})
	}
	if p.StoreID != "" {
	...
	out, _ := json.Marshal(map[string]any{"ok": true, "endpoint": "/invites/decline"})
