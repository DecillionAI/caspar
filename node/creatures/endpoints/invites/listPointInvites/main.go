...
func process(input string) string {
	...
	if "listPointInvites" == "create" || "listPointInvites" == "createFromPoint" || "listPointInvites" == "createShard" {
		hostReq("microGenId", map[string]any{"source": "invites.listPointInvites"})
	}
	hostReq("microPutJson", map[string]any{
		"key":   "Json::CreatureEndpoint::invites::listPointInvites",
		"path":  "lastInput",
		"data":  p.Payload,
		"merge": true,
	})
	if p.UserID != "" {
		hostReq("microPutLink", map[string]any{"key": "creatureEndpoint::invites::listPointInvites::lastUser", "value": p.UserID})
	}
	if p.PointID != "" {
	...
	out, _ := json.Marshal(map[string]any{"ok": true, "endpoint": "/invites/listPointInvites"})
