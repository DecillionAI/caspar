...
func process(input string) string {
	...
	if "listStoreInvites" == "create" || "listStoreInvites" == "createFromStore" || "listStoreInvites" == "createShard" {
		hostReq("microGenId", map[string]any{"source": "invites.listStoreInvites"})
	}
	...
	if p.UserID != "" {
		hostReq("microPutLink", map[string]any{"key": "creatureEndpoint::invites::listStoreInvites::lastUser", "value": p.UserID})
	}
	if p.StoreID != "" {
	...
	out, _ := json.Marshal(map[string]any{"ok": true, "endpoint": "/invites/listStoreInvites"})
