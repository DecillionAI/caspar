...
func process(input string) string {
	...
	if "execCommand" == "create" || "execCommand" == "createFromPoint" || "execCommand" == "createShard" {
		hostReq("microGenId", map[string]any{"source": "pc.execCommand"})
	}
	hostReq("microPutJson", map[string]any{
		"key":   "Json::CreatureEndpoint::pc::execCommand",
		"path":  "lastInput",
		"data":  p.Payload,
		"merge": true,
	})
	if p.UserID != "" {
		hostReq("microPutLink", map[string]any{"key": "creatureEndpoint::pc::execCommand::lastUser", "value": p.UserID})
	}
	if p.PointID != "" {
	...
	out, _ := json.Marshal(map[string]any{"ok": true, "endpoint": "/pc/execCommand"})
