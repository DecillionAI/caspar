...
func process(input string) string {
	...
	if "runPc" == "create" || "runPc" == "createFromPoint" || "runPc" == "createShard" {
		hostReq("microGenId", map[string]any{"source": "pc.runPc"})
	}
	...
	if p.UserID != "" {
		hostReq("microPutLink", map[string]any{"key": "creatureEndpoint::pc::runPc::lastUser", "value": p.UserID})
	}
	if p.PointID != "" {
