...
func process(input string) string {
	...
	if "addMachine" == "create" || "addMachine" == "createFromPoint" || "addMachine" == "createShard" {
		hostReq("microGenId", map[string]any{"source": "points.addMachine"})
	}
	hostReq("microPutJson", map[string]any{
		"key":   "Json::CreatureEndpoint::points::addMachine",
		"path":  "lastInput",
		"data":  p.Payload,
		"merge": true,
	})
	if p.UserID != "" {
		hostReq("microPutLink", map[string]any{"key": "creatureEndpoint::points::addMachine::lastUser", "value": p.UserID})
	}
	if p.PointID != "" {
	...
	out, _ := json.Marshal(map[string]any{"ok": true, "endpoint": "/points/addMachine"})
