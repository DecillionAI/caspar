...
func process(input string) string {
	...
	if "addMember" == "create" || "addMember" == "createFromPoint" || "addMember" == "createShard" {
		hostReq("microGenId", map[string]any{"source": "points.addMember"})
	}
	hostReq("microPutJson", map[string]any{
		"key":   "Json::CreatureEndpoint::points::addMember",
		"path":  "lastInput",
		"data":  p.Payload,
		"merge": true,
	})
	if p.UserID != "" {
		hostReq("microPutLink", map[string]any{"key": "creatureEndpoint::points::addMember::lastUser", "value": p.UserID})
	}
	if p.PointID != "" {
	...
	out, _ := json.Marshal(map[string]any{"ok": true, "endpoint": "/points/addMember"})
