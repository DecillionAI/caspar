...
func process(input string) string {
	...
	if "addProgram" == "create" || "addProgram" == "createFromPoint" || "addProgram" == "createShard" {
		hostReq("microGenId", map[string]any{"source": "points.addProgram"})
	}
	hostReq("microPutJson", map[string]any{
		"key":   "Json::CreatureEndpoint::points::addProgram",
		"path":  "lastInput",
		"data":  p.Payload,
		"merge": true,
	})
	if p.UserID != "" {
		hostReq("microPutLink", map[string]any{"key": "creatureEndpoint::points::addProgram::lastUser", "value": p.UserID})
	}
	if p.PointID != "" {
	...
	out, _ := json.Marshal(map[string]any{"ok": true, "endpoint": "/points/addProgram"})
