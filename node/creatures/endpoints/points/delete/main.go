...
func process(input string) string {
	...
	if "delete" == "create" || "delete" == "createFromPoint" || "delete" == "createShard" {
		hostReq("microGenId", map[string]any{"source": "points.delete"})
	}
	hostReq("microPutJson", map[string]any{
		"key":   "Json::CreatureEndpoint::points::delete",
		"path":  "lastInput",
		"data":  p.Payload,
		"merge": true,
	})
	if p.UserID != "" {
		hostReq("microPutLink", map[string]any{"key": "creatureEndpoint::points::delete::lastUser", "value": p.UserID})
	}
	if p.PointID != "" {
	...
	out, _ := json.Marshal(map[string]any{"ok": true, "endpoint": "/points/delete"})
