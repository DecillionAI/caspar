...
func process(input string) string {
	...
	if "getDefaultAccess" == "create" || "getDefaultAccess" == "createFromPoint" || "getDefaultAccess" == "createShard" {
		hostReq("microGenId", map[string]any{"source": "points.getDefaultAccess"})
	}
	hostReq("microPutJson", map[string]any{
		"key":   "Json::CreatureEndpoint::points::getDefaultAccess",
		"path":  "lastInput",
		"data":  p.Payload,
		"merge": true,
	})
	if p.UserID != "" {
		hostReq("microPutLink", map[string]any{"key": "creatureEndpoint::points::getDefaultAccess::lastUser", "value": p.UserID})
	}
	if p.PointID != "" {
	...
	out, _ := json.Marshal(map[string]any{"ok": true, "endpoint": "/points/getDefaultAccess"})
