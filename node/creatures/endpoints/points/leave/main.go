...
func process(input string) string {
	...
	if "leave" == "create" || "leave" == "createFromPoint" || "leave" == "createShard" {
		hostReq("microGenId", map[string]any{"source": "points.leave"})
	}
	hostReq("microPutJson", map[string]any{
		"key":   "Json::CreatureEndpoint::points::leave",
		"path":  "lastInput",
		"data":  p.Payload,
		"merge": true,
	})
	if p.UserID != "" {
		hostReq("microPutLink", map[string]any{"key": "creatureEndpoint::points::leave::lastUser", "value": p.UserID})
	}
	if p.PointID != "" {
	...
	out, _ := json.Marshal(map[string]any{"ok": true, "endpoint": "/points/leave"})
