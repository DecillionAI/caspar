...
func process(input string) string {
	...
	if "listPointMachines" == "create" || "listPointMachines" == "createFromPoint" || "listPointMachines" == "createShard" {
		hostReq("microGenId", map[string]any{"source": "points.listPointMachines"})
	}
	hostReq("microPutJson", map[string]any{
		"key":   "Json::CreatureEndpoint::points::listPointMachines",
		"path":  "lastInput",
		"data":  p.Payload,
		"merge": true,
	})
	if p.UserID != "" {
		hostReq("microPutLink", map[string]any{"key": "creatureEndpoint::points::listPointMachines::lastUser", "value": p.UserID})
	}
	if p.PointID != "" {
	...
	out, _ := json.Marshal(map[string]any{"ok": true, "endpoint": "/points/listPointMachines"})
