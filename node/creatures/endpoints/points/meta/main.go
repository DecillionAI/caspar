...
func process(input string) string {
	...
	if "meta" == "create" || "meta" == "createFromPoint" || "meta" == "createShard" {
		hostReq("microGenId", map[string]any{"source": "points.meta"})
	}
	hostReq("microPutJson", map[string]any{
		"key":   "Json::CreatureEndpoint::points::meta",
		"path":  "lastInput",
		"data":  p.Payload,
		"merge": true,
	})
	if p.UserID != "" {
		hostReq("microPutLink", map[string]any{"key": "creatureEndpoint::points::meta::lastUser", "value": p.UserID})
	}
	if p.PointID != "" {
	...
	out, _ := json.Marshal(map[string]any{"ok": true, "endpoint": "/points/meta"})
