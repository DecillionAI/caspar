...
func process(input string) string {
	...
	if "readMembers" == "create" || "readMembers" == "createFromPoint" || "readMembers" == "createShard" {
		hostReq("microGenId", map[string]any{"source": "points.readMembers"})
	}
	hostReq("microPutJson", map[string]any{
		"key":   "Json::CreatureEndpoint::points::readMembers",
		"path":  "lastInput",
		"data":  p.Payload,
		"merge": true,
	})
	if p.UserID != "" {
		hostReq("microPutLink", map[string]any{"key": "creatureEndpoint::points::readMembers::lastUser", "value": p.UserID})
	}
	if p.PointID != "" {
	...
	out, _ := json.Marshal(map[string]any{"ok": true, "endpoint": "/points/readMembers"})
