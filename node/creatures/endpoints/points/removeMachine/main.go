...
func process(input string) string {
	...
	if "removeMachine" == "create" || "removeMachine" == "createFromPoint" || "removeMachine" == "createShard" {
		hostReq("microGenId", map[string]any{"source": "points.removeMachine"})
	}
	hostReq("microPutJson", map[string]any{
		"key":   "Json::CreatureEndpoint::points::removeMachine",
		"path":  "lastInput",
		"data":  p.Payload,
		"merge": true,
	})
	if p.UserID != "" {
		hostReq("microPutLink", map[string]any{"key": "creatureEndpoint::points::removeMachine::lastUser", "value": p.UserID})
	}
	if p.PointID != "" {
	...
	out, _ := json.Marshal(map[string]any{"ok": true, "endpoint": "/points/removeMachine"})
