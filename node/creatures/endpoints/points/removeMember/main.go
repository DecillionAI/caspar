...
func process(input string) string {
	...
	if "removeMember" == "create" || "removeMember" == "createFromPoint" || "removeMember" == "createShard" {
		hostReq("microGenId", map[string]any{"source": "points.removeMember"})
	}
	hostReq("microPutJson", map[string]any{
		"key":   "Json::CreatureEndpoint::points::removeMember",
		"path":  "lastInput",
		"data":  p.Payload,
		"merge": true,
	})
	if p.UserID != "" {
		hostReq("microPutLink", map[string]any{"key": "creatureEndpoint::points::removeMember::lastUser", "value": p.UserID})
	}
	if p.PointID != "" {
	...
	out, _ := json.Marshal(map[string]any{"ok": true, "endpoint": "/points/removeMember"})
