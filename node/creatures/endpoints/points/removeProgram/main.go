...
func process(input string) string {
	...
	if "removeProgram" == "create" || "removeProgram" == "createFromPoint" || "removeProgram" == "createShard" {
		hostReq("microGenId", map[string]any{"source": "points.removeProgram"})
	}
	hostReq("microPutJson", map[string]any{
		"key":   "Json::CreatureEndpoint::points::removeProgram",
		"path":  "lastInput",
		"data":  p.Payload,
		"merge": true,
	})
	if p.UserID != "" {
		hostReq("microPutLink", map[string]any{"key": "creatureEndpoint::points::removeProgram::lastUser", "value": p.UserID})
	}
	if p.PointID != "" {
	...
	out, _ := json.Marshal(map[string]any{"ok": true, "endpoint": "/points/removeProgram"})
