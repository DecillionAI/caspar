...
func process(input string) string {
	...
	if "signal" == "create" || "signal" == "createFromPoint" || "signal" == "createShard" {
		hostReq("microGenId", map[string]any{"source": "points.signal"})
	}
	hostReq("microPutJson", map[string]any{
		"key":   "Json::CreatureEndpoint::points::signal",
		"path":  "lastInput",
		"data":  p.Payload,
		"merge": true,
	})
	if p.UserID != "" {
		hostReq("microPutLink", map[string]any{"key": "creatureEndpoint::points::signal::lastUser", "value": p.UserID})
	}
	if p.PointID != "" {
	...
	out, _ := json.Marshal(map[string]any{"ok": true, "endpoint": "/points/signal"})
