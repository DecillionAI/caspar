...
func process(input string) string {
	...
	if "updateMember" == "create" || "updateMember" == "createFromPoint" || "updateMember" == "createShard" {
		hostReq("microGenId", map[string]any{"source": "points.updateMember"})
	}
	hostReq("microPutJson", map[string]any{
		"key":   "Json::CreatureEndpoint::points::updateMember",
		"path":  "lastInput",
		"data":  p.Payload,
		"merge": true,
	})
	if p.UserID != "" {
		hostReq("microPutLink", map[string]any{"key": "creatureEndpoint::points::updateMember::lastUser", "value": p.UserID})
	}
	if p.PointID != "" {
	...
	out, _ := json.Marshal(map[string]any{"ok": true, "endpoint": "/points/updateMember"})
