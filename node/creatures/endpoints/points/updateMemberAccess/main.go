...
func process(input string) string {
	...
	if "updateMemberAccess" == "create" || "updateMemberAccess" == "createFromPoint" || "updateMemberAccess" == "createShard" {
		hostReq("microGenId", map[string]any{"source": "points.updateMemberAccess"})
	}
	hostReq("microPutJson", map[string]any{
		"key":   "Json::CreatureEndpoint::points::updateMemberAccess",
		"path":  "lastInput",
		"data":  p.Payload,
		"merge": true,
	})
	if p.UserID != "" {
		hostReq("microPutLink", map[string]any{"key": "creatureEndpoint::points::updateMemberAccess::lastUser", "value": p.UserID})
	}
	if p.PointID != "" {
	...
	out, _ := json.Marshal(map[string]any{"ok": true, "endpoint": "/points/updateMemberAccess"})
