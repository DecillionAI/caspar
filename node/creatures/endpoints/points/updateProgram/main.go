...
func process(input string) string {
	...
	if "updateProgram" == "create" || "updateProgram" == "createFromPoint" || "updateProgram" == "createShard" {
		hostReq("microGenId", map[string]any{"source": "points.updateProgram"})
	}
	hostReq("microPutJson", map[string]any{
		"key":   "Json::CreatureEndpoint::points::updateProgram",
		"path":  "lastInput",
		"data":  p.Payload,
		"merge": true,
	})
	if p.UserID != "" {
		hostReq("microPutLink", map[string]any{"key": "creatureEndpoint::points::updateProgram::lastUser", "value": p.UserID})
	}
	if p.PointID != "" {
	...
	out, _ := json.Marshal(map[string]any{"ok": true, "endpoint": "/points/updateProgram"})
