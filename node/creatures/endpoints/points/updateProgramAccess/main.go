...
func process(input string) string {
	...
	if "updateProgramAccess" == "create" || "updateProgramAccess" == "createFromPoint" || "updateProgramAccess" == "createShard" {
		hostReq("microGenId", map[string]any{"source": "points.updateProgramAccess"})
	}
	hostReq("microPutJson", map[string]any{
		"key":   "Json::CreatureEndpoint::points::updateProgramAccess",
		"path":  "lastInput",
		"data":  p.Payload,
		"merge": true,
	})
	if p.UserID != "" {
		hostReq("microPutLink", map[string]any{"key": "creatureEndpoint::points::updateProgramAccess::lastUser", "value": p.UserID})
	}
	if p.PointID != "" {
	...
	out, _ := json.Marshal(map[string]any{"ok": true, "endpoint": "/points/updateProgramAccess"})
