...
func process(input string) string {
	...
	if "deletePointEntity" == "create" || "deletePointEntity" == "createFromPoint" || "deletePointEntity" == "createShard" {
		hostReq("microGenId", map[string]any{"source": "storage.deletePointEntity"})
	}
	hostReq("microPutJson", map[string]any{
		"key":   "Json::CreatureEndpoint::storage::deletePointEntity",
		"path":  "lastInput",
		"data":  p.Payload,
		"merge": true,
	})
	if p.UserID != "" {
		hostReq("microPutLink", map[string]any{"key": "creatureEndpoint::storage::deletePointEntity::lastUser", "value": p.UserID})
	}
	if p.PointID != "" {
	...
	out, _ := json.Marshal(map[string]any{"ok": true, "endpoint": "/storage/deletePointEntity"})
