...
func process(input string) string {
	...
	if "deleteStoreEntity" == "create" || "deleteStoreEntity" == "createFromStore" || "deleteStoreEntity" == "createShard" {
		hostReq("microGenId", map[string]any{"source": "storage.deleteStoreEntity"})
	}
	...
	if p.UserID != "" {
		hostReq("microPutLink", map[string]any{"key": "creatureEndpoint::storage::deleteStoreEntity::lastUser", "value": p.UserID})
	}
	if p.StoreID != "" {
