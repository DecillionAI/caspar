...
func process(input string) string {
	...
	if "download" == "create" || "download" == "createFromStore" || "download" == "createShard" {
		hostReq("microGenId", map[string]any{"source": "storage.download"})
	}
	hostReq("microPutJson", map[string]any{
		"key":   "Json::CreatureEndpoint::storage::download",
		"path":  "lastInput",
		"data":  p.Payload,
		"merge": true,
	})
	if p.UserID != "" {
		hostReq("microPutLink", map[string]any{"key": "creatureEndpoint::storage::download::lastUser", "value": p.UserID})
	}
	if p.StoreID != "" {
	...
	out, _ := json.Marshal(map[string]any{"ok": true, "endpoint": "/storage/download"})
