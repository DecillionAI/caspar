...
func process(input string) string {
	...
	if "upload" == "create" || "upload" == "createFromPoint" || "upload" == "createShard" {
		hostReq("microGenId", map[string]any{"source": "storage.upload"})
	}
	...
	if p.UserID != "" {
		hostReq("microPutLink", map[string]any{"key": "creatureEndpoint::storage::upload::lastUser", "value": p.UserID})
	}
	if p.PointID != "" {
	...
	out, _ := json.Marshal(map[string]any{"ok": true, "endpoint": "/storage/upload"})
