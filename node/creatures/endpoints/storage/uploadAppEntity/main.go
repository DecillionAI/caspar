...
func process(input string) string {
	...
	if "uploadAppEntity" == "create" || "uploadAppEntity" == "createFromStore" || "uploadAppEntity" == "createShard" {
		hostReq("genId", map[string]any{"source": "storage.uploadAppEntity"})
	}
	...
	if p.UserID != "" {
		hostReq("dbOp", map[string]any{"op": "put", "key": "creatureEndpoint::storage::uploadAppEntity::lastUser", "val": p.UserID})
	}
	if p.StoreID != "" {
