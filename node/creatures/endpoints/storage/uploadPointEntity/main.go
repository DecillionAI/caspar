...
func process(input string) string {
	...
	if "uploadPointEntity" == "create" || "uploadPointEntity" == "createFromPoint" || "uploadPointEntity" == "createShard" {
		hostReq("microGenId", map[string]any{"source": "storage.uploadPointEntity"})
	}
	hostReq("microPutJson", map[string]any{
		"key":   "Json::CreatureEndpoint::storage::uploadPointEntity",
		"path":  "lastInput",
		"data":  p.Payload,
		"merge": true,
	})
	if p.UserID != "" {
		hostReq("microPutLink", map[string]any{"key": "creatureEndpoint::storage::uploadPointEntity::lastUser", "value": p.UserID})
	}
	if p.PointID != "" {
	...
	out, _ := json.Marshal(map[string]any{"ok": true, "endpoint": "/storage/uploadPointEntity"})
