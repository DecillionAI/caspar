...
func process(input string) string {
	...
	if "uploadStoreEntity" == "create" || "uploadStoreEntity" == "createFromStore" || "uploadStoreEntity" == "createShard" {
		hostReq("microGenId", map[string]any{"source": "storage.uploadStoreEntity"})
	}
	...
	if p.UserID != "" {
		hostReq("microPutLink", map[string]any{"key": "creatureEndpoint::storage::uploadStoreEntity::lastUser", "value": p.UserID})
	}
	if p.StoreID != "" {
