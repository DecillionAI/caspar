...
func process(input string) string {
	...
	if "addMember" == "create" || "addMember" == "createFromStore" || "addMember" == "createShard" {
		hostReq("microGenId", map[string]any{"source": "stores.addMember"})
	}
	hostReq("microPutJson", map[string]any{
		"key":   "Json::CreatureEndpoint::stores::addMember",
		"path":  "lastInput",
		"data":  p.Payload,
		"merge": true,
	})
	if p.UserID != "" {
		hostReq("microPutLink", map[string]any{"key": "creatureEndpoint::stores::addMember::lastUser", "value": p.UserID})
	}
	if p.StoreID != "" {
	...
	out, _ := json.Marshal(map[string]any{"ok": true, "endpoint": "/stores/addMember"})
