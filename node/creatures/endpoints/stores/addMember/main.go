...
func process(input string) string {
	...
	if "addMember" == "create" || "addMember" == "createFromSpace" || "addMember" == "createShard" {
		hostReq("microGenId", map[string]any{"source": "spaces.addMember"})
	}
	hostReq("microPutJson", map[string]any{
		"key":   "Json::CreatureEndpoint::spaces::addMember",
		"path":  "lastInput",
		"data":  p.Payload,
		"merge": true,
	})
	if p.UserID != "" {
		hostReq("microPutLink", map[string]any{"key": "creatureEndpoint::spaces::addMember::lastUser", "value": p.UserID})
	}
	if p.SpaceID != "" {
	...
	out, _ := json.Marshal(map[string]any{"ok": true, "endpoint": "/spaces/addMember"})
