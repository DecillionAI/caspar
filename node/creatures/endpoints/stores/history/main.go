...
func process(input string) string {
	...
	if "history" == "create" || "history" == "createFromStore" || "history" == "createShard" {
		hostReq("microGenId", map[string]any{"source": "stores.history"})
	}
	hostReq("microPutJson", map[string]any{
		"key":   "Json::CreatureEndpoint::stores::history",
		"path":  "lastInput",
		"data":  p.Payload,
		"merge": true,
	})
	if p.UserID != "" {
		hostReq("microPutLink", map[string]any{"key": "creatureEndpoint::stores::history::lastUser", "value": p.UserID})
	}
	if p.StoreID != "" {
	...
	out, _ := json.Marshal(map[string]any{"ok": true, "endpoint": "/stores/history"})
