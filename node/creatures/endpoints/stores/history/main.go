...
func process(input string) string {
	...
	if "history" == "create" || "history" == "createFromSpace" || "history" == "createShard" {
		hostReq("microGenId", map[string]any{"source": "spaces.history"})
	}
	...
	if p.UserID != "" {
		hostReq("microPutLink", map[string]any{"key": "creatureEndpoint::spaces::history::lastUser", "value": p.UserID})
	}
	if p.SpaceID != "" {
	...
	out, _ := json.Marshal(map[string]any{"ok": true, "endpoint": "/spaces/history"})
