...
func process(input string) string {
	...
	if "join" == "create" || "join" == "createFromStore" || "join" == "createShard" {
		hostReq("microGenId", map[string]any{"source": "stores.join"})
	}
	hostReq("microPutJson", map[string]any{
		"key":   "Json::CreatureEndpoint::stores::join",
		"path":  "lastInput",
		"data":  p.Payload,
		"merge": true,
	})
	if p.UserID != "" {
		hostReq("microPutLink", map[string]any{"key": "creatureEndpoint::stores::join::lastUser", "value": p.UserID})
	}
	if p.StoreID != "" {
	...
	out, _ := json.Marshal(map[string]any{"ok": true, "endpoint": "/stores/join"})
