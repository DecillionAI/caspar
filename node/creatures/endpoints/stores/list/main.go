...
func process(input string) string {
	...
	if "list" == "create" || "list" == "createFromSpace" || "list" == "createShard" {
		hostReq("microGenId", map[string]any{"source": "spaces.list"})
	}
	...
	if p.UserID != "" {
		hostReq("microPutLink", map[string]any{"key": "creatureEndpoint::spaces::list::lastUser", "value": p.UserID})
	}
	if p.SpaceID != "" {
	...
	out, _ := json.Marshal(map[string]any{"ok": true, "endpoint": "/spaces/list"})
