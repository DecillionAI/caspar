...
func process(input string) string {
	...
	if "listSpaceMachines" == "create" || "listSpaceMachines" == "createFromSpace" || "listSpaceMachines" == "createShard" {
		hostReq("microGenId", map[string]any{"source": "spaces.listSpaceMachines"})
	}
	...
	if p.UserID != "" {
		hostReq("microPutLink", map[string]any{"key": "creatureEndpoint::spaces::listSpaceMachines::lastUser", "value": p.UserID})
	}
	if p.SpaceID != "" {
	...
	out, _ := json.Marshal(map[string]any{"ok": true, "endpoint": "/spaces/listSpaceMachines"})
