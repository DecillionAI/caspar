...
func process(input string) string {
	...
	if "listStoreMachines" == "create" || "listStoreMachines" == "createFromStore" || "listStoreMachines" == "createShard" {
		hostReq("microGenId", map[string]any{"source": "stores.listStoreMachines"})
	}
	hostReq("microPutJson", map[string]any{
		"key":   "Json::CreatureEndpoint::stores::listStoreMachines",
		"path":  "lastInput",
		"data":  p.Payload,
		"merge": true,
	})
	if p.UserID != "" {
		hostReq("microPutLink", map[string]any{"key": "creatureEndpoint::stores::listStoreMachines::lastUser", "value": p.UserID})
	}
	if p.StoreID != "" {
	...
	out, _ := json.Marshal(map[string]any{"ok": true, "endpoint": "/stores/listStoreMachines"})
