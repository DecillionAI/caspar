...
func process(input string) string {
	...
	if "meta" == "create" || "meta" == "createFromStore" || "meta" == "createShard" {
		hostReq("microGenId", map[string]any{"source": "stores.meta"})
	}
	hostReq("microPutJson", map[string]any{
		"key":   "Json::CreatureEndpoint::stores::meta",
		"path":  "lastInput",
		"data":  p.Payload,
		"merge": true,
	})
	if p.UserID != "" {
		hostReq("microPutLink", map[string]any{"key": "creatureEndpoint::stores::meta::lastUser", "value": p.UserID})
	}
	if p.StoreID != "" {
	...
	out, _ := json.Marshal(map[string]any{"ok": true, "endpoint": "/stores/meta"})
