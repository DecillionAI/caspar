...
func process(input string) string {
	...
	if "meta" == "create" || "meta" == "createFromSpace" || "meta" == "createShard" {
		hostReq("microGenId", map[string]any{"source": "spaces.meta"})
	}
	hostReq("microPutJson", map[string]any{
		"key":   "Json::CreatureEndpoint::spaces::meta",
		"path":  "lastInput",
		"data":  p.Payload,
		"merge": true,
	})
	if p.UserID != "" {
		hostReq("microPutLink", map[string]any{"key": "creatureEndpoint::spaces::meta::lastUser", "value": p.UserID})
	}
	if p.SpaceID != "" {
	...
	out, _ := json.Marshal(map[string]any{"ok": true, "endpoint": "/spaces/meta"})
