...
func process(input string) string {
	...
	if "read" == "create" || "read" == "createFromSpace" || "read" == "createShard" {
		hostReq("microGenId", map[string]any{"source": "spaces.read"})
	}
	...
	if p.UserID != "" {
		hostReq("microPutLink", map[string]any{"key": "creatureEndpoint::spaces::read::lastUser", "value": p.UserID})
	}
	if p.SpaceID != "" {
	...
	out, _ := json.Marshal(map[string]any{"ok": true, "endpoint": "/spaces/read"})
