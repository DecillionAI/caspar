...
func process(input string) string {
	...
	if "readMembers" == "create" || "readMembers" == "createFromSpace" || "readMembers" == "createShard" {
		hostReq("genId", map[string]any{"source": "spaces.readMembers"})
	}
	...
	if p.UserID != "" {
		hostReq("dbOp", map[string]any{"op": "put", "key": "creatureEndpoint::spaces::readMembers::lastUser", "val": p.UserID})
	}
	if p.SpaceID != "" {
