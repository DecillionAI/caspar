...
func process(input string) string {
	...
	if "removeMachine" == "create" || "removeMachine" == "createFromSpace" || "removeMachine" == "createShard" {
		hostReq("microGenId", map[string]any{"source": "spaces.removeMachine"})
	}
	hostReq("microPutJson", map[string]any{
		"key":   "Json::CreatureEndpoint::spaces::removeMachine",
		"path":  "lastInput",
		"data":  p.Payload,
		"merge": true,
	})
	if p.UserID != "" {
		hostReq("microPutLink", map[string]any{"key": "creatureEndpoint::spaces::removeMachine::lastUser", "value": p.UserID})
	}
	if p.SpaceID != "" {
	...
	out, _ := json.Marshal(map[string]any{"ok": true, "endpoint": "/spaces/removeMachine"})
