...
func process(input string) string {
	...
	if "removeMachine" == "create" || "removeMachine" == "createFromStore" || "removeMachine" == "createShard" {
		hostReq("microGenId", map[string]any{"source": "stores.removeMachine"})
	}
	hostReq("microPutJson", map[string]any{
		"key":   "Json::CreatureEndpoint::stores::removeMachine",
		"path":  "lastInput",
		"data":  p.Payload,
		"merge": true,
	})
	if p.UserID != "" {
		hostReq("microPutLink", map[string]any{"key": "creatureEndpoint::stores::removeMachine::lastUser", "value": p.UserID})
	}
	if p.StoreID != "" {
	...
	out, _ := json.Marshal(map[string]any{"ok": true, "endpoint": "/stores/removeMachine"})
