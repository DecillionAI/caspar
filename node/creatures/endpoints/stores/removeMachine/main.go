...
func process(input string) string {
	...
	if "removeMachine" == "create" || "removeMachine" == "createFromSpace" || "removeMachine" == "createShard" {
		hostReq("genId", map[string]any{"source": "spaces.removeMachine"})
	}
	...
	if p.UserID != "" {
		hostReq("dbOp", map[string]any{"op": "put", "key": "creatureEndpoint::spaces::removeMachine::lastUser", "val": p.UserID})
	}
	if p.SpaceID != "" {
