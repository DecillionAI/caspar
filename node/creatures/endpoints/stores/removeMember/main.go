...
func process(input string) string {
	...
	if "removeMember" == "create" || "removeMember" == "createFromSpace" || "removeMember" == "createShard" {
		hostReq("genId", map[string]any{"source": "spaces.removeMember"})
	}
	...
	if p.UserID != "" {
		hostReq("dbOp", map[string]any{"op": "put", "key": "creatureEndpoint::spaces::removeMember::lastUser", "val": p.UserID})
	}
	if p.SpaceID != "" {
