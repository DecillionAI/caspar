...
func process(input string) string {
	...
	if "removeProgram" == "create" || "removeProgram" == "createFromStore" || "removeProgram" == "createShard" {
		hostReq("microGenId", map[string]any{"source": "stores.removeProgram"})
	}
	hostReq("microPutJson", map[string]any{
		"key":   "Json::CreatureEndpoint::stores::removeProgram",
		"path":  "lastInput",
		"data":  p.Payload,
		"merge": true,
	})
	if p.UserID != "" {
		hostReq("microPutLink", map[string]any{"key": "creatureEndpoint::stores::removeProgram::lastUser", "value": p.UserID})
	}
	if p.StoreID != "" {
	...
	out, _ := json.Marshal(map[string]any{"ok": true, "endpoint": "/stores/removeProgram"})
