...
func process(input string) string {
	...
	if "removeProgram" == "create" || "removeProgram" == "createFromSpace" || "removeProgram" == "createShard" {
		hostReq("microGenId", map[string]any{"source": "spaces.removeProgram"})
	}
	hostReq("microPutJson", map[string]any{
		"key":   "Json::CreatureEndpoint::spaces::removeProgram",
		"path":  "lastInput",
		"data":  p.Payload,
		"merge": true,
	})
	if p.UserID != "" {
		hostReq("microPutLink", map[string]any{"key": "creatureEndpoint::spaces::removeProgram::lastUser", "value": p.UserID})
	}
	if p.SpaceID != "" {
	...
	out, _ := json.Marshal(map[string]any{"ok": true, "endpoint": "/spaces/removeProgram"})
