...
func process(input string) string {
	...
	if "removeProgram" == "create" || "removeProgram" == "createFromSpace" || "removeProgram" == "createShard" {
		hostReq("genId", map[string]any{"source": "spaces.removeProgram"})
	}
	...
	if p.UserID != "" {
		hostReq("dbOp", map[string]any{"op": "put", "key": "creatureEndpoint::spaces::removeProgram::lastUser", "val": p.UserID})
	}
	if p.SpaceID != "" {
