...
func process(input string) string {
	...
	if "updateMemberAccess" == "create" || "updateMemberAccess" == "createFromStore" || "updateMemberAccess" == "createShard" {
		hostReq("microGenId", map[string]any{"source": "stores.updateMemberAccess"})
	}
	hostReq("microPutJson", map[string]any{
		"key":   "Json::CreatureEndpoint::stores::updateMemberAccess",
		"path":  "lastInput",
		"data":  p.Payload,
		"merge": true,
	})
	if p.UserID != "" {
		hostReq("microPutLink", map[string]any{"key": "creatureEndpoint::stores::updateMemberAccess::lastUser", "value": p.UserID})
	}
	if p.StoreID != "" {
	...
	out, _ := json.Marshal(map[string]any{"ok": true, "endpoint": "/stores/updateMemberAccess"})
