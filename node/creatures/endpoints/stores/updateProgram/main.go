...
func process(input string) string {
	...
	if "updateProgram" == "create" || "updateProgram" == "createFromSpace" || "updateProgram" == "createShard" {
		hostReq("genId", map[string]any{"source": "spaces.updateProgram"})
	}
	...
	if p.UserID != "" {
		hostReq("dbOp", map[string]any{"op": "put", "key": "creatureEndpoint::spaces::updateProgram::lastUser", "val": p.UserID})
	}
	if p.SpaceID != "" {
