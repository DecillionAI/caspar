...
func process(input string) string {
	...
	if "updateProgram" == "create" || "updateProgram" == "createFromSpace" || "updateProgram" == "createShard" {
		hostReq("microGenId", map[string]any{"source": "spaces.updateProgram"})
	}
	hostReq("microPutJson", map[string]any{
		"key":   "Json::CreatureEndpoint::spaces::updateProgram",
		"path":  "lastInput",
		"data":  p.Payload,
		"merge": true,
	})
	if p.UserID != "" {
		hostReq("microPutLink", map[string]any{"key": "creatureEndpoint::spaces::updateProgram::lastUser", "value": p.UserID})
	}
	if p.SpaceID != "" {
	...
	out, _ := json.Marshal(map[string]any{"ok": true, "endpoint": "/spaces/updateProgram"})
