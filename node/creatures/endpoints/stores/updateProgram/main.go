...
func process(input string) string {
	...
	if "updateProgram" == "create" || "updateProgram" == "createFromStore" || "updateProgram" == "createShard" {
		hostReq("microGenId", map[string]any{"source": "stores.updateProgram"})
	}
	hostReq("microPutJson", map[string]any{
		"key":   "Json::CreatureEndpoint::stores::updateProgram",
		"path":  "lastInput",
		"data":  p.Payload,
		"merge": true,
	})
	if p.UserID != "" {
		hostReq("microPutLink", map[string]any{"key": "creatureEndpoint::stores::updateProgram::lastUser", "value": p.UserID})
	}
	if p.StoreID != "" {
	...
	out, _ := json.Marshal(map[string]any{"ok": true, "endpoint": "/stores/updateProgram"})
