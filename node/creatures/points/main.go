...
func process(input string) string {
	...
	hostReq("microPutJson", map[string]any{"key": "Json::CreatureNamespace::points", "path": "lastInput", "data": p.Payload, "merge": true})
	if p.Path != "" {
		hostReq("microPutLink", map[string]any{"key": "creatureNamespace::points::lastPath", "value": p.Path})
	}
	out, _ := json.Marshal(map[string]any{"ok": true, "namespace": "points"})
