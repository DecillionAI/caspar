//! Translation of `abstract/adapters/file/file.go`.
...
pub trait IFile: Send + Sync {
    ...
    /// Go passes `*tar.Reader` positioned at an entry; the entry implements
    /// `io.Reader`, so the translation accepts the reader directly.
