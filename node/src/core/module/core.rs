...
use crate::core::module::actor::{Actor, Info as BaseInfo, State as ActorState};
use crate::core::module::actor::model::trx::TrxWrapper;
use crate::core::module::globe::{ChainPacketOp, Globe};
...
impl Core {
    ...
    pub fn load_inner(
        ...
        *self.gods.lock().unwrap() = gods;
        ...
        let chain: Arc<dyn crate::abstractions::adapters::network::chain::IChain> =
        ...
        *self.chain_tx.lock().unwrap() = Some(chain_tx.clone());
        ...
        let peers_fn = {
            ...
            Arc::new(move || net.chain().peers()) as crate::core::module::globe::PeersFn
        };
        let sign_fn: crate::core::module::globe::SignPacketFn = {
            let me = self.clone();
            Arc::new(move |data| me.sign_packet(data))
        };
        let submit_fn: crate::core::module::globe::SubmitChainPacketFn = {
        ...
        let set_chain_callback_fn: crate::core::module::globe::SetChainCallbackFn = {
        ...
        let set_message_cb_fn: crate::core::module::globe::SetMessageCbFn = {
        ...
        *self.globe.lock().unwrap() = Some(globe.clone());
        ...
        // `handle_chain_packet`.
        let trans = self.clone();
        let pipeline: crate::abstractions::adapters::network::chain::PipelineFn =
