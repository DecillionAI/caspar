...
use crate::drivers::vmm::appengine::globals::{GLOBAL_DB, GLOBAL_RESOURCE_LOCKS, ResourceLockState, ResourceLockEntry};
use crate::drivers::vmm::appengine::models::runtime_models::{Trx, WasmDbOp};
use crate::drivers::vmm::appengine::bridge::messaging::{wasm_send, log, set_log_vm_context};
use crate::drivers::vmm::appengine::host::task_graph::host_call;
...
impl WasmMac {
    ...
    pub fn finalize(&mut self) -> Vec<WasmDbOp> {
        ...
        if self.has_output {
            ...
            crate::drivers::vmm::appengine::bridge::messaging::wasm_send(payload);
        }
        self.trx.ops.clone()
    }
