...
/// Hard cap on queued signals per machine, so a flood at a dead creature cannot
/// grow unbounded between sweeps. Oldest are dropped first.
const PENDING_CAP: usize = 256;
/// While a cold-spawn is in flight for a machine, further signals only queue —
...
/// A signal captured while its target machine had no live gateway connection,
...
pub(crate) struct GatewayRegistry {
    ...
    next_id: AtomicU64,
    /// Signals captured while a machine had no live connection, keyed by machine
    /// id and flushed in FIFO order the moment a container connects.
    pending: DashMap<String, VecDeque<PendingSignal>>,
    /// Machines with a cold-spawn currently in flight (debounce), so concurrent
    /// signals boot the container once, not once each.
...
impl GatewayRegistry {
    ...
    /// The single newest live connection for a machine, or `None`. Cold-spawn
    ...
    fn newest_conn(&self, machine_id: &str) -> Option<Arc<GatewayConnection>> {
        self.by_machine(machine_id)
            .into_iter()
            .max_by_key(|c| c.conn_id)
    }

    /// Capture a signal for a machine that has no live connection yet, so it can
    /// be delivered once the container (re)connects. Bounded by `PENDING_CAP`
    ...
    pub(crate) fn queue_pending_signal(&self, machine_id: &str, key: &str, data: &JsonValue) {
        {
            let mut q = self.pending.entry(machine_id.to_string()).or_default();
        ...
        if self.newest_conn(machine_id).is_some() {
            self.flush_pending_signals(machine_id);
        }
    }

    /// Deliver every queued signal for `machine_id` to its live container, in the
    ...
    /// deliver the same packet twice. Returns the number delivered. A machine with
    /// no live connection keeps its queue for the next connect.
    pub(crate) fn flush_pending_signals(&self, machine_id: &str) -> usize {
        let Some(conn) = self.newest_conn(machine_id) else {
            return 0;
        };
        let Some((_, queued)) = self.pending.remove(machine_id) else {
            return 0;
        };
        let mut delivered = 0;
        for sig in queued {
    ...
    /// Claim the cold-spawn slot for a machine. Returns `true` for the caller that
    ...
    /// that never connected can be retried later.
    pub(crate) fn begin_cold_spawn(&self, machine_id: &str) -> bool {
        let now = Instant::now();
        match self.spawning.entry(machine_id.to_string()) {
    ...
    /// period may spawn again immediately.
    pub(crate) fn clear_cold_spawn(&self, machine_id: &str) {
        self.spawning.remove(machine_id);
    }
