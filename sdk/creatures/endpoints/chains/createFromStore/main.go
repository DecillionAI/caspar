...
func process(input string) string {
	...
	hostReq("putJson", map[string]any{
		"key":   "Json::CreatureEndpoint::chains::createFromStore",
		"path":  "lastInput",
		"data":  p.Payload,
		"merge": true,
	})
	packetBytes, _ := json.Marshal(map[string]any{"endpoint": "/chains/createFromStore", "payload": p.Payload})
	signalKey := "chains/createFromStore"
	if p.CreatureID != "" {
		hostReq("dbOp", map[string]any{"op": "put", "key": "creatureEndpoint::chains::createFromStore::lastCreature", "val": p.CreatureID})
	}
	if p.StoreID != "" {
	...
	if chainID == "" {
		...
		if chainID == "" {
			genRes := hostReq("genId", map[string]any{"source": "chains.createFromStore.chain"})
			chainID = hostResString(genRes, "id")
		}
	}
	shardChainID, _ := p.Payload["shardChainId"].(string)
	peers := p.Payload["peers"]
	if peers == nil {
		peers = []string{}
	}
	if shardChainID == "" {
		createRes := hostReq("createShardChain", map[string]any{"chainId": chainID, "shardChainId": shardChainID, "peers": peers})
		shardChainID = hostResString(createRes, "shardChainId", "id")
		if shardChainID == "" {
			genRes := hostReq("genId", map[string]any{"source": "chains.createFromStore.shard"})
			shardChainID = hostResString(genRes, "id")
		}
	}
	...
	out, _ := json.Marshal(map[string]any{"chainId": chainID, "shardChainId": shardChainID, "peers": peers, "storeId": storeID})
