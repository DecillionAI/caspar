...
func process(input string) string {
	...
	if "createFromStore" == "create" || "createFromStore" == "createFromStore" || "createFromStore" == "createShard" {
		hostReq("genId", map[string]any{"source": "chains.createFromStore"})
	}
	...
	if p.UserID != "" {
		hostReq("dbOp", map[string]any{"op": "put", "key": "creatureEndpoint::chains::createFromStore::lastUser", "val": p.UserID})
	}
	if p.StoreID != "" {
