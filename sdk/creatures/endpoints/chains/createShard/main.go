...
func process(input string) string {
	...
	hostReq("putJson", map[string]any{
		"key":   "Json::CreatureEndpoint::chains::createShard",
		"path":  "lastInput",
		"data":  p.Payload,
		"merge": true,
	})
	packetBytes, _ := json.Marshal(map[string]any{"endpoint": "/chains/createShard", "payload": p.Payload})
	signalKey := "chains/createShard"
	if p.CreatureID != "" {
		hostReq("dbOp", map[string]any{"op": "put", "key": "creatureEndpoint::chains::createShard::lastCreature", "val": p.CreatureID})
	}
	if p.StoreID != "" {
	...
	if shardChainID == "" {
		...
		if shardChainID == "" {
			genRes := hostReq("genId", map[string]any{"source": "chains.createShard.shard"})
			shardChainID = hostResString(genRes, "id")
		}
	}
	out, _ := json.Marshal(map[string]any{"chainId": chainID, "shardChainId": shardChainID})
