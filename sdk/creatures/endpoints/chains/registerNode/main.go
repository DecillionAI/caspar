...
func process(input string) string {
	...
	if "registerNode" == "create" || "registerNode" == "createFromStore" || "registerNode" == "createShard" {
		hostReq("genId", map[string]any{"source": "chains.registerNode"})
	}
	...
	if p.UserID != "" {
		hostReq("dbOp", map[string]any{"op": "put", "key": "creatureEndpoint::chains::registerNode::lastUser", "val": p.UserID})
	}
	if p.StoreID != "" {
