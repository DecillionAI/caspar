...
func process(input string) string {
	...
	packetBytes, _ := json.Marshal(map[string]any{"endpoint": "/chains/submitBaseTrx", "payload": p.Payload})
	signalKey := "chains/submitBaseTrx"
	if p.CreatureID != "" {
		hostReq("dbOp", map[string]any{"op": "put", "key": "creatureEndpoint::chains::submitBaseTrx::lastCreature", "val": p.CreatureID})
	}
	if p.StoreID != "" {
