...
func process(input string) string {
	...
	packetBytes, _ := json.Marshal(map[string]any{"endpoint": "/invites/cancel", "payload": p.Payload})
	signalKey := "invites/cancel"
	if p.CreatureID != "" {
		hostReq("dbOp", map[string]any{"op": "put", "key": "creatureEndpoint::invites::cancel::lastCreature", "val": p.CreatureID})
	}
	if p.StoreID != "" {
