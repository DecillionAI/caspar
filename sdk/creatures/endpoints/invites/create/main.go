...
func process(input string) string {
	...
	packetBytes, _ := json.Marshal(map[string]any{"endpoint": "/invites/create", "payload": p.Payload})
	signalKey := "invites/create"
	if p.CreatureID != "" {
		hostReq("dbOp", map[string]any{"op": "put", "key": "creatureEndpoint::invites::create::lastCreature", "val": p.CreatureID})
	}
	if p.StoreID != "" {
