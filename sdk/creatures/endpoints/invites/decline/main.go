...
func process(input string) string {
	...
	packetBytes, _ := json.Marshal(map[string]any{"endpoint": "/invites/decline", "payload": p.Payload})
	signalKey := "invites/decline"
	if p.CreatureID != "" {
		hostReq("dbOp", map[string]any{"op": "put", "key": "creatureEndpoint::invites::decline::lastCreature", "val": p.CreatureID})
	}
	if p.StoreID != "" {
