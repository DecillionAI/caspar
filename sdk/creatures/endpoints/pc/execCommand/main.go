...
func process(input string) string {
	...
	packetBytes, _ := json.Marshal(map[string]any{"endpoint": "/pc/execCommand", "payload": p.Payload})
	signalKey := "pc/execCommand"
	if p.CreatureID != "" {
		hostReq("dbOp", map[string]any{"op": "put", "key": "creatureEndpoint::pc::execCommand::lastCreature", "val": p.CreatureID})
	}
	if p.StoreID != "" {
