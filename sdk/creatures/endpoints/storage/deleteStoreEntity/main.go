...
func process(input string) string {
	...
	packetBytes, _ := json.Marshal(map[string]any{"endpoint": "/storage/deleteStoreEntity", "payload": p.Payload})
	signalKey := "storage/deleteStoreEntity"
	if p.CreatureID != "" {
		hostReq("dbOp", map[string]any{"op": "put", "key": "creatureEndpoint::storage::deleteStoreEntity::lastCreature", "val": p.CreatureID})
	}
	if p.StoreID != "" {
