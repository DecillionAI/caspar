...
func process(input string) string {
	...
	packetBytes, _ := json.Marshal(map[string]any{"endpoint": "/storage/deleteUserEntity", "payload": p.Payload})
	signalKey := "storage/deleteUserEntity"
	if p.CreatureID != "" {
		hostReq("dbOp", map[string]any{"op": "put", "key": "creatureEndpoint::storage::deleteUserEntity::lastCreature", "val": p.CreatureID})
	}
	if p.StoreID != "" {
