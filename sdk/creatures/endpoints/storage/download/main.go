...
func process(input string) string {
	...
	packetBytes, _ := json.Marshal(map[string]any{"endpoint": "/storage/download", "payload": p.Payload})
	signalKey := "storage/download"
	if p.CreatureID != "" {
		hostReq("dbOp", map[string]any{"op": "put", "key": "creatureEndpoint::storage::download::lastCreature", "val": p.CreatureID})
	}
	if p.StoreID != "" {
