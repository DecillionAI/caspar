...
func process(input string) string {
	...
	hostReq("putJson", map[string]any{
		"key":   "Json::CreatureEndpoint::storage::upload",
		"path":  "lastInput",
		"data":  p.Payload,
		"merge": true,
	})
	packetBytes, _ := json.Marshal(map[string]any{"endpoint": "/storage/upload", "payload": p.Payload})
	signalKey := "storage/upload"
	if p.CreatureID != "" {
		hostReq("dbOp", map[string]any{"op": "put", "key": "creatureEndpoint::storage::upload::lastCreature", "val": p.CreatureID})
	}
	if p.StoreID != "" {
	...
	if file, ok := p.Payload["file"]; ok && file != nil {
		out, _ := json.Marshal(map[string]any{"file": file})
