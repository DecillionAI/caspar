...
func process(input string) string {
	...
	hostReq("putJson", map[string]any{
		"key":   "Json::CreatureEndpoint::storage::uploadAppEntity",
		"path":  "lastInput",
		"data":  p.Payload,
		"merge": true,
	})
	packetBytes, _ := json.Marshal(map[string]any{"endpoint": "/storage/uploadAppEntity", "payload": p.Payload})
	signalKey := "storage/uploadAppEntity"
	if p.CreatureID != "" {
		hostReq("dbOp", map[string]any{"op": "put", "key": "creatureEndpoint::storage::uploadAppEntity::lastCreature", "val": p.CreatureID})
	}
	if p.StoreID != "" {
