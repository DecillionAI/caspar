...
func process(input string) string {
	...
	packetBytes, _ := json.Marshal(map[string]any{"endpoint": "/storage/uploadUserEntity", "payload": p.Payload})
	signalKey := "storage/uploadUserEntity"
	if p.CreatureID != "" {
		hostReq("dbOp", map[string]any{"op": "put", "key": "creatureEndpoint::storage::uploadUserEntity::lastCreature", "val": p.CreatureID})
	}
	if p.StoreID != "" {
