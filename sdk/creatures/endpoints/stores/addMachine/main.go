...
func process(input string) string {
	...
	packetBytes, _ := json.Marshal(map[string]any{"endpoint": "/spaces/addMachine", "payload": p.Payload})
	signalKey := "spaces/addMachine"
	if p.CreatureID != "" {
		hostReq("dbOp", map[string]any{"op": "put", "key": "creatureEndpoint::spaces::addMachine::lastCreature", "val": p.CreatureID})
	}
	if p.SpaceID != "" {
