...
func process(input string) string {
	...
	packetBytes, _ := json.Marshal(map[string]any{"endpoint": "/spaces/addMember", "payload": p.Payload})
	signalKey := "spaces/addMember"
	if p.CreatureID != "" {
		hostReq("dbOp", map[string]any{"op": "put", "key": "creatureEndpoint::spaces::addMember::lastCreature", "val": p.CreatureID})
	}
	if p.SpaceID != "" {
