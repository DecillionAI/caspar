...
func process(input string) string {
	...
	packetBytes, _ := json.Marshal(map[string]any{"endpoint": "/spaces/addProgram", "payload": p.Payload})
	signalKey := "spaces/addProgram"
	if p.CreatureID != "" {
		hostReq("dbOp", map[string]any{"op": "put", "key": "creatureEndpoint::spaces::addProgram::lastCreature", "val": p.CreatureID})
	}
	if p.SpaceID != "" {
