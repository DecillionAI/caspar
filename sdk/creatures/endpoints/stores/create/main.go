...
func process(input string) string {
	...
	packetBytes, _ := json.Marshal(map[string]any{"endpoint": "/spaces/create", "payload": p.Payload})
	signalKey := "spaces/create"
	if p.CreatureID != "" {
		hostReq("dbOp", map[string]any{"op": "put", "key": "creatureEndpoint::spaces::create::lastCreature", "val": p.CreatureID})
	}
	if p.SpaceID != "" {
