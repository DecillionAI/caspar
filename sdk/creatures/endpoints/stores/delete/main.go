...
func process(input string) string {
	...
	packetBytes, _ := json.Marshal(map[string]any{"endpoint": "/spaces/delete", "payload": p.Payload})
	signalKey := "spaces/delete"
	if p.CreatureID != "" {
		hostReq("dbOp", map[string]any{"op": "put", "key": "creatureEndpoint::spaces::delete::lastCreature", "val": p.CreatureID})
	}
	if p.SpaceID != "" {
