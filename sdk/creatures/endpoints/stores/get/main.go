...
func process(input string) string {
	...
	packetBytes, _ := json.Marshal(map[string]any{"endpoint": "/spaces/get", "payload": p.Payload})
	signalKey := "spaces/get"
	if p.CreatureID != "" {
		hostReq("dbOp", map[string]any{"op": "put", "key": "creatureEndpoint::spaces::get::lastCreature", "val": p.CreatureID})
	}
	if p.SpaceID != "" {
	...
	out, _ := json.Marshal(map[string]any{"store": store})
