...
func process(input string) string {
	...
	hostReq("putJson", map[string]any{
		"key":   "Json::CreatureEndpoint::spaces::getDefaultAccess",
		"path":  "lastInput",
		"data":  p.Payload,
		"merge": true,
	})
	packetBytes, _ := json.Marshal(map[string]any{"endpoint": "/spaces/getDefaultAccess", "payload": p.Payload})
	signalKey := "spaces/getDefaultAccess"
	if p.CreatureID != "" {
		hostReq("dbOp", map[string]any{"op": "put", "key": "creatureEndpoint::spaces::getDefaultAccess::lastCreature", "val": p.CreatureID})
	}
	if p.SpaceID != "" {
	...
	access := p.Payload["access"]
	if access == nil {
		access = map[string]bool{"createSubStore": false, "deleteSubStore": false, "uploadEntity": false, "updateMetadata": true, "sendSignal": true, "readHistory": true, "addMachine": true, "addProgram": true, "updateProgram": true, "removeProgram": true, "removeMachine": true, "addMember": false, "updateMember": false, "readMembers": false, "removeMember": false}
	}
	out, _ := json.Marshal(map[string]any{"access": access})
