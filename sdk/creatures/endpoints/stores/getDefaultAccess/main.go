...
func process(input string) string {
	...
	if "getDefaultAccess" == "create" || "getDefaultAccess" == "createFromSpace" || "getDefaultAccess" == "createShard" {
		hostReq("genId", map[string]any{"source": "spaces.getDefaultAccess"})
	}
	...
	if p.UserID != "" {
		hostReq("dbOp", map[string]any{"op": "put", "key": "creatureEndpoint::spaces::getDefaultAccess::lastUser", "val": p.UserID})
	}
	if p.SpaceID != "" {
