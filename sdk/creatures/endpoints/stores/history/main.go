...
func process(input string) string {
	...
	hostReq("putJson", map[string]any{
		"key":   "Json::CreatureEndpoint::spaces::history",
		"path":  "lastInput",
		"data":  p.Payload,
		"merge": true,
	})
	packetBytes, _ := json.Marshal(map[string]any{"endpoint": "/spaces/history", "payload": p.Payload})
	signalKey := "spaces/history"
	if p.CreatureID != "" {
		hostReq("dbOp", map[string]any{"op": "put", "key": "creatureEndpoint::spaces::history::lastCreature", "val": p.CreatureID})
	}
	if p.SpaceID != "" {
	...
	packets := p.Payload["packets"]
	if packets == nil {
		packets = []map[string]any{}
	}
	out, _ := json.Marshal(map[string]any{"packets": packets})
