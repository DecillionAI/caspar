...
func process(input string) string {
	...
	packetBytes, _ := json.Marshal(map[string]any{"endpoint": "/spaces/join", "payload": p.Payload})
	signalKey := "spaces/join"
	if p.CreatureID != "" {
		hostReq("dbOp", map[string]any{"op": "put", "key": "creatureEndpoint::spaces::join::lastCreature", "val": p.CreatureID})
	}
	if p.SpaceID != "" {
