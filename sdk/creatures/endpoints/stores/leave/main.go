...
func process(input string) string {
	...
	packetBytes, _ := json.Marshal(map[string]any{"endpoint": "/spaces/leave", "payload": p.Payload})
	signalKey := "spaces/leave"
	if p.CreatureID != "" {
		hostReq("dbOp", map[string]any{"op": "put", "key": "creatureEndpoint::spaces::leave::lastCreature", "val": p.CreatureID})
	}
	if p.SpaceID != "" {
