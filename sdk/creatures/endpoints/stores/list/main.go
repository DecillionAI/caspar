...
func process(input string) string {
	...
	hostReq("putJson", map[string]any{
		"key":   "Json::CreatureEndpoint::spaces::list",
		"path":  "lastInput",
		"data":  p.Payload,
		"merge": true,
	})
	packetBytes, _ := json.Marshal(map[string]any{"endpoint": "/spaces/list", "payload": p.Payload})
	signalKey := "spaces/list"
	if p.CreatureID != "" {
		hostReq("dbOp", map[string]any{"op": "put", "key": "creatureEndpoint::spaces::list::lastCreature", "val": p.CreatureID})
	}
	if p.SpaceID != "" {
