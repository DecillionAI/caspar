...
func process(input string) string {
	...
	packetBytes, _ := json.Marshal(map[string]any{"endpoint": "/spaces/meta", "payload": p.Payload})
	signalKey := "spaces/meta"
	if p.CreatureID != "" {
		hostReq("dbOp", map[string]any{"op": "put", "key": "creatureEndpoint::spaces::meta::lastCreature", "val": p.CreatureID})
	}
	if p.SpaceID != "" {
