...
func process(input string) string {
	...
	packetBytes, _ := json.Marshal(map[string]any{"endpoint": "/spaces/readMembers", "payload": p.Payload})
	signalKey := "spaces/readMembers"
	if p.CreatureID != "" {
		hostReq("dbOp", map[string]any{"op": "put", "key": "creatureEndpoint::spaces::readMembers::lastCreature", "val": p.CreatureID})
	}
	if p.SpaceID != "" {
	...
	members := p.Payload["members"]
	if members == nil {
		members = []map[string]any{}
	}
	out, _ := json.Marshal(map[string]any{"members": members})
