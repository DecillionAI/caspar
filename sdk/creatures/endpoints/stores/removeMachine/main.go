...
func process(input string) string {
	...
	packetBytes, _ := json.Marshal(map[string]any{"endpoint": "/spaces/removeMachine", "payload": p.Payload})
	signalKey := "spaces/removeMachine"
	if p.CreatureID != "" {
		hostReq("dbOp", map[string]any{"op": "put", "key": "creatureEndpoint::spaces::removeMachine::lastCreature", "val": p.CreatureID})
	}
	if p.SpaceID != "" {
