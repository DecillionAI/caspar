...
func process(input string) string {
	...
	packetBytes, _ := json.Marshal(map[string]any{"endpoint": "/spaces/removeMember", "payload": p.Payload})
	signalKey := "spaces/removeMember"
	if p.CreatureID != "" {
		hostReq("dbOp", map[string]any{"op": "put", "key": "creatureEndpoint::spaces::removeMember::lastCreature", "val": p.CreatureID})
	}
	if p.SpaceID != "" {
