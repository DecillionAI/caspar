...
func process(input string) string {
	...
	packetBytes, _ := json.Marshal(map[string]any{"endpoint": "/spaces/removeProgram", "payload": p.Payload})
	signalKey := "spaces/removeProgram"
	if p.CreatureID != "" {
		hostReq("dbOp", map[string]any{"op": "put", "key": "creatureEndpoint::spaces::removeProgram::lastCreature", "val": p.CreatureID})
	}
	if p.SpaceID != "" {
