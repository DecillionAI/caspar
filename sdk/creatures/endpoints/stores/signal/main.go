...
func process(input string) string {
	...
	hostReq("putJson", map[string]any{
		"key":   "Json::CreatureEndpoint::spaces::signal",
		"path":  "lastInput",
		"data":  p.Payload,
		"merge": true,
	})
	packetBytes, _ := json.Marshal(map[string]any{"endpoint": "/spaces/signal", "payload": p.Payload})
	signalKey := "spaces/signal"
	if p.CreatureID != "" {
		hostReq("dbOp", map[string]any{"op": "put", "key": "creatureEndpoint::spaces::signal::lastCreature", "val": p.CreatureID})
	}
	if p.SpaceID != "" {
	...
	packet := p.Payload["packet"]
	if packet == nil {
		packet = map[string]any{}
	}
	passed, ok := p.Payload["passed"].(bool)
	if !ok {
		passed = false
	}
	out, _ := json.Marshal(map[string]any{"packet": packet, "passed": passed})
