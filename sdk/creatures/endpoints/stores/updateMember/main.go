...
func process(input string) string {
	...
	packetBytes, _ := json.Marshal(map[string]any{"endpoint": "/spaces/updateMember", "payload": p.Payload})
	signalKey := "spaces/updateMember"
	if p.CreatureID != "" {
		hostReq("dbOp", map[string]any{"op": "put", "key": "creatureEndpoint::spaces::updateMember::lastCreature", "val": p.CreatureID})
	}
	if p.SpaceID != "" {
	...
	out, _ := json.Marshal(map[string]any{"metadata": metadata})
