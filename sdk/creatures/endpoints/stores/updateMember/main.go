...
func process(input string) string {
	...
	if "updateMember" == "create" || "updateMember" == "createFromSpace" || "updateMember" == "createShard" {
		hostReq("genId", map[string]any{"source": "spaces.updateMember"})
	}
	...
	if p.UserID != "" {
		hostReq("dbOp", map[string]any{"op": "put", "key": "creatureEndpoint::spaces::updateMember::lastUser", "val": p.UserID})
	}
	if p.SpaceID != "" {
