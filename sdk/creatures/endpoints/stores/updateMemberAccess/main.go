...
func process(input string) string {
	...
	packetBytes, _ := json.Marshal(map[string]any{"endpoint": "/spaces/updateMemberAccess", "payload": p.Payload})
	signalKey := "spaces/updateMemberAccess"
	if p.CreatureID != "" {
		hostReq("dbOp", map[string]any{"op": "put", "key": "creatureEndpoint::spaces::updateMemberAccess::lastCreature", "val": p.CreatureID})
	}
	if p.SpaceID != "" {
