...
func process(input string) string {
	...
	packetBytes, _ := json.Marshal(map[string]any{"endpoint": "/spaces/updateProgram", "payload": p.Payload})
	signalKey := "spaces/updateProgram"
	if p.CreatureID != "" {
		hostReq("dbOp", map[string]any{"op": "put", "key": "creatureEndpoint::spaces::updateProgram::lastCreature", "val": p.CreatureID})
	}
	if p.SpaceID != "" {
