...
func process(input string) string {
	...
	packetBytes, _ := json.Marshal(map[string]any{"endpoint": "/spaces/updateProgramAccess", "payload": p.Payload})
	signalKey := "spaces/updateProgramAccess"
	if p.CreatureID != "" {
		hostReq("dbOp", map[string]any{"op": "put", "key": "creatureEndpoint::spaces::updateProgramAccess::lastCreature", "val": p.CreatureID})
	}
	if p.SpaceID != "" {
